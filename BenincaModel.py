...
def main():
    global m,p
    m = BenincaModel(Es=Es_normal,Ps='auto/Beninca_set3.hdf5',Vs=None)
    return 0
...
#            self.gradmat=create_gradient(self.setup['n'],self.setup['l'], self.setup['bc'] , [1.0,self.p['Dw'],self.p['Dh']])
    ...
    def set_equations(self):
        ...
        if self.setup['rhs']=="Beninca_forced_logistic":
            ...
            self.dB0dt_eq = p['cBR']*(B0+BA)*R-p['cAB']*A*B0-p['cM']*M*B0-p['mB']*B0+Ft*p['mA']*BA
            self.dBAdt_eq = p['cAB']*A*B0-p['cM']*M*BA-p['mB']*BA-Ft*p['mA']*BA
            self.dAdt_eq  = (p['cAR']*A*R+p['cAB']*A*B0-p['cM']*M*A-Ft*p['mA']*A)*(1.0-A**4)
            self.dMdt_eq  = (p['cM']*M*(B0+A)-Ft*p['mM']*M)*(1.0-M**4)
            self.Ft_sym=Ft
